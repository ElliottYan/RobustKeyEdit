...
class TraceInput(Trace):
    def __init__(
        ...
        self.registered_hook = module.register_forward_pre_hook(retain_hook)
        self.stop = stop
...
class TraceInputDict(OrderedDict, contextlib.AbstractContextManager):
    """
    To retain the output of multiple named layers during the computation
    of the given network:
    ...
    def __init__(
        ...
        for is_last, layer in flag_last_unseen(layers):
            if edit_input_dict is not None:
                assert edit_input is None
                cur_edit_input = edit_input_dict[layer]
            else:
                cur_edit_input = edit_input
                
            self[layer] = TraceInput(
                module=module,
                layer=layer,
                retain_output=retain_output,
                retain_input=retain_input,
...
def get_module(model, name):
    ...
    for n, m in model.named_modules():
        if n == name:
            return m
    # guess the model is wrapped by peft model
