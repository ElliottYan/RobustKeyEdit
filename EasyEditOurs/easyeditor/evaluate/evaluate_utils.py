...
@torch.no_grad()
def run_lprobs(model, tokenizer, sentence, device, prefix=None, return_token_list=False):
    # whether it has sos or eos? GPT2 has neither...
    if prefix:
        # breakpoint()
        prefix_ids = tokenizer(prefix, return_tensors="pt").to(device)["input_ids"]
        target_ids = tokenizer(sentence, return_tensors="pt").to(device)
        targets_ids = target_ids["input_ids"][:, 1:] # remove bos
        inps = torch.concat((prefix_ids, targets_ids), dim=-1)
        input_ids = inps[:,:-1]
    ...
    # NOTE: truncation not applied? why sentence or new_sentence?
        target_ids = tokenizer(sentence, return_tensors='pt').to(device)
        targets_ids = target_ids["input_ids"][:,1:]
        inps = tokenizer(sentence, return_tensors='pt', max_length=2048, truncation=True).to(device)
    ...
    if return_token_list:
        return gather_lprobs.squeeze().tolist(), sent_lprobs.detach().cpu()
...
def test_prediction_acc_ours(model, tok, hparams, prompts, targets, device, locality=False):
    if isinstance(prompts, str):
        prompts,targets = [prompts,], [targets,]
    prompt_target = [prompt + ' ' + target for prompt, target in zip(prompts,targets)]
    prompt_target_tok = tok(prompt_target)['input_ids']
    prompts_tok = tok(prompts)['input_ids']
    prompt_len = [len(item) for item in prompts_tok]
    prompt_target_len = [len(item) for item in prompt_target_tok]
    max_len = max(prompt_target_len)
    bsz = len(prompts)
    inputs = torch.zeros([bsz, max_len]).fill_(tok.pad_token_id).long()
    for i in range(bsz):
        inputs[i][:prompt_target_len[i]] = torch.tensor(prompt_target_tok[i]).long()
    inputs = inputs.to(f"cuda:{device}")
    mask = inputs != tok.pad_token_id
    with torch.no_grad():
        ...
        labels = inputs.squeeze(-1).detach().cpu().numpy().tolist()
        answers = slice_list(answers,prompt_len,left=True)
        labels = slice_list(labels,prompt_len,left=False)
        # now the answers and labels contain a lot of paddings! we should filter them!
        # filter according to labels
        filter_labels = []
        for k in range(bsz):
            try:
                label = [x for x in labels[k] if x != tok.pad_token_id]
            except:
                breakpoint()
            filter_labels.append(label)
        filter_answer = [answers[k][:len(filter_labels[k])] for k in range(bsz)]
        answers = filter_answer
        ...
        if isinstance(answers[0], list):
            ...
            for ans,label in zip(answers,labels):
                temp_acc = np.min(np.equal(ans, label)).astype(float) # only exact match
                if np.isnan(temp_acc):
                    continue
                res.append(temp_acc)
            return res
        else:
            return [np.min(np.equal(answers, labels)).astype(float)] # only exact match
