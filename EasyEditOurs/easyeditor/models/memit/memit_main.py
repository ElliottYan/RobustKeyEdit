...
def apply_memit_to_model(
    ...
    **kwargs
    ...
    deltas, layer_ks = execute_memit(model, tok, requests, hparams, cache_template=cache_template)
    print("!!!!#@#!")
    with torch.no_grad():
    ...
    if hparams.use_our_aggregator:
        from ..aggregator import Aggregator
        model = Aggregator(hparams, model, tok, device='cuda')
        assert len(requests) == 1
        model.edit(requests[0], layer_ks)
...
def execute_memit(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        # D = D + 0.1* torch.eye(D.shape[0], dtype=D.dtype, device = D.device)#to counter ill-conditioned D
        # try:
        ...
        resid = targets / (len(hparams.layers) - i)  # Distribute residual across layers [4096,1]
        upd_matrix = resid @ adj_k.T #[4096, 11008]
    ...
    return deltas, torch.cat(layer_kss, dim=1).transpose(0, 1) # [N, D]
